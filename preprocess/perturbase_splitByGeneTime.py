...
def extract_drug_name_with_dose(gene_value):
    """从gene值中提取基因名称，保留剂量信息但进行文件名安全处理"""
...
def process_time_column(adata, time_column=None):
    """处理时间列，提取数字部分作为时间值，非数字部分作为单位"""
    if time_column and time_column in adata.obs.columns:
        print(f"处理时间列: {time_column}")
        
        # 初始化新的列
        adata.obs['time'] = 0.0
        adata.obs['unit'] = ''
        
        for idx, time_value in adata.obs[time_column].items():
            if pd.isna(time_value) or time_value == 'CTRL':
                # CTRL或空值设为0，单位为空
                adata.obs.at[idx, 'time'] = 0.0
                adata.obs.at[idx, 'unit'] = ''
            else:
                # 将时间值转换为字符串
                ...
                if time_match:
                    time_num = float(time_match.group())
                    adata.obs.at[idx, 'time'] = time_num
                    
                    # 提取非数字部分作为单位，并去除空格
                    unit_part = re.sub(r'[-+]?\d*\.?\d+', '', time_str).strip()
                    # 去除多余空格
                    unit_part = re.sub(r'\s+', '', unit_part)
                    adata.obs.at[idx, 'unit'] = unit_part
                else:
                    # 如果没有找到数字，设为1（默认处理）
                    adata.obs.at[idx, 'time'] = 1.0
                    adata.obs.at[idx, 'unit'] = time_str
    else:
        # 如果没有提供时间列或列不存在，使用原来的逻辑
        print("使用默认时间处理逻辑")
        adata.obs['time'] = 0  # 默认为0
        for idx, gene_value in adata.obs['gene'].items():
            if gene_value != 'CTRL':
                adata.obs.at[idx, 'time'] = 1
    
    return adata

def split_h5ad_by_gene_with_dose(input_file, output_prefix, min_cells=500, time_column=None):
    """根据gene列拆分h5ad文件，保留剂量信息"""
    
    # 读取h5ad文件，忽略观察名和变量名不唯一的警告
    ...
    # 处理时间列
    adata = process_time_column(adata, time_column)
    ...
    print(f"找到 {len(drug_combinations)} 个基因组合")
    ...
    for drug_str, indices in drug_combinations.items():
        ...
        if len(indices) < min_cells:
            print(f"跳过 {drug_str}: 只有 {len(indices)} 个基因细胞 (< {min_cells})")
            skipped_due_to_min_cells += 1
            continue
        
        # 创建子集：包含该基因组合的所有细胞和所有CTRL细胞
        # 使用集合去重，然后转换为列表
    ...
    print(f"跳过 {skipped_due_to_min_cells} 个基因组合 (细胞数 < {min_cells})")
    print("生成的文件:")
...
def main():
    ...
    parser.add_argument('output_prefix', help='输出文件前缀，格式为"前缀.基因名称.time.h5ad"')
    parser.add_argument('--min_cells', type=int, default=500, help='最小基因细胞数阈值，默认500')
    parser.add_argument('-t', '--timepoints', help='时间点列名，从中提取数字作为时间值，非数字作为单位')
    ...
    try:
        split_h5ad_by_gene_with_dose(args.input_file, args.output_prefix, args.min_cells, args.timepoints)

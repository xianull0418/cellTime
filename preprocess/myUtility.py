...
def annData_to_tigon_format(adata, time_column='time', embedding_key='X_AE', sample_ratio=1.0):
    """
    将 AnnData 对象转换为 TIGON 要求的精确格式

    返回: numpy 数组, shape = (1, n_time_points)
          每个元素是 (n_cells, n_dimensions) 的数组
    """

    # 1. 获取低维嵌入
    lowdim_data = adata.obsm[embedding_key]

    # 2. 获取时间信息并分组
    time_info = adata.obs[time_column]

    # 将时间点转换为数值（如果还不是数值的话）
    if time_info.dtype == 'object':
        unique_times = sorted(time_info.unique())
        time_mapping = {t: i for i, t in enumerate(unique_times)}
        numeric_times = time_info.map(time_mapping).values
        print(f"时间点映射: {time_mapping}")
    else:
        numeric_times = time_info.values
        unique_times = sorted(np.unique(numeric_times))

    # 剔除 NaN
    unique_times = np.array(unique_times, dtype=np.float64)
    unique_times = unique_times[~np.isnan(unique_times)].tolist()

    # 3. 按时间点分组
    time_arrays = []
    for t in unique_times:
        mask = numeric_times == t
        cells_at_time = lowdim_data[mask]
        
        n_cells = cells_at_time.shape[0]
        n_sample = max(1, int(n_cells * sample_ratio))  # 确保至少1个细胞
        
        # 随机采样
        sampled_indices = np.random.choice(n_cells, size=n_sample, replace=False)
        sampled_cells = cells_at_time[sampled_indices]

        time_arrays.append(sampled_cells)
        print(f"时间点 {t}: {cells_at_time.shape} -> {sampled_cells.shape}")


    sys.stdout.flush()

    # 4. 创建 TIGON 格式: (1, n_time_points) 的 object 数组
    tigon_data = np.empty((1, len(time_arrays)), dtype=object)
    for i, arr in enumerate(time_arrays):
        tigon_data[0, i] = arr

    return tigon_data, unique_times
...
def create_args2():
    ...
    parser.add_argument('--timepoints', type=str, default=None,
                        help='Time points of data (default: extracted from adata.obs["time"])')
    ...
    parser.add_argument('--num_samples', type=int, default=100,
                        help='Number of sampling points per epoch (default: 100)')

    parser.add_argument('--hidden_dim', type=int, default=16,
                        help='Dimension of the hidden layer (default: 16)')

    parser.add_argument('--n_hiddens', type=int, default=4,
    ...
    parser.add_argument('--gpu', type=int, default=None,
                        help='GPU device index (default: None)')

    parser.add_argument('--input_h5ad', type=str, default=None,
                        help='Input Files Directory (default: None)')
    ...
    parser.add_argument('--sample_ratio', type=float, default=None,
                        help='sample_ratio for downsampling cells (default: cell_n > 2000 ? 2000/cell_n : 1.0) ')

    args = parser.parse_args()

    # 处理timepoints参数，将其从字符串转换为浮点数列表
    if args.timepoints is not None:
        args.timepoints = [float(tp.strip()) for tp in args.timepoints.split(",")]
...
def loaddata2(args,device):
    if args.input_h5ad is not None:
        adata = sc.read_h5ad(args.input_h5ad)

        if args.sample_ratio is None:
            if adata.obs.shape[0] > 2000:
                args.sample_ratio = float( 2000 / adata.obs.shape[0] )
            else:
                args.sample_ratio = 1.0

        data, time_points = annData_to_tigon_format(
            adata,
            time_column='time',
            embedding_key='X_AE',

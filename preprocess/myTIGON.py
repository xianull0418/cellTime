...
if __name__ == '__main__':
    ...
    # load dataset
    data_train = loaddata2(args,device)
    integral_time = args.timepoints

    # scale to [0, 10]
#    integral_time = np.array(integral_time, dtype=np.float64)
#    integral_time = integral_time[~np.isnan(integral_time)]
#
#    if integral_time.max() > 100000.0:
#        print(f'scale timepoints to [0, 10]')
#        scaled_time = 100000.0 * integral_time  / integral_time.max()
#        integral_time = scaled_time
#    integral_time = integral_time.tolist()
#    print(f'timepoints: {integral_time}', flush=True)
